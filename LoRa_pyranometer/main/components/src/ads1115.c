...
#ifdef ADS1115_SENSOR // wrapped the code with this for easy deactivation of this device when not using it for debug purposes

#include "ads1115.h"

ads_t ads_ir = {0};      // we are using the ADS1115 to calculate irradiance, this line initializes a struct with this data
...
static esp_err_t ads1115_read_register(ads1115_t *ads, ads1115_register_addresses_t reg, uint8_t *data, uint8_t len)
{
    ...
    if (ads->last_reg != reg)
    { // if we're not on the correct register, change it
        cmd = i2c_cmd_link_create();
...
void ads1115_set_rdy_pin(ads1115_t *ads, gpio_num_t gpio)
{
    ...
    io_conf.intr_type = GPIO_INTR_NEGEDGE; // positive to negative (pulled down) **** another change for modern pattern
...
int16_t ads1115_get_raw(ads1115_t *ads)
{
    ...
    { // if it's single-ended or a setting changed
...
/** Task to initialize and the device and loop for data gathering **/
void ads1115_task(void *pvParameters)
{
    ads1115_fsr_t _fsr = ADS1115_FSR_6_144;                  // initializes the full-scale resolution to 6.144 volts
    ads1115_mode_t _mode = ADS1115_MODE_SINGLE;              // initializes the mode to single
    ads1115_t _ads = ads1115_config(I2C_PORT, ADS1115_ADDR); // initializes the device
    ...
    uint16_t count = 0;                                      // initializes a variable to count how many increments are being made
    int64_t start = esp_timer_get_time();                    // initializes a time control variable with the current time
    while (1)
    {
        volt += ads1115_get_voltage(&_ads);                      // accumulates the voltage reading
        count++;                                                 // increments the count variable
        if (esp_timer_get_time() - start >= TEMPO_ANALISE * 1e6) // if it passed the time we set to receive data
        {
            volt = volt / count;                                     // calculates the mean of the voltage accumulated
            ads_ir.irrad = volt / (RF * (PD_AREA * 1e-6) * PD_SENS); // calculates the irradiance
            xQueueOverwrite(ads_queue, &ads_ir);                     // sends the data through the data transmission queue
            volt = 0;                                                // resets the voltage variable
            count = 0;                                               // and the count variable
            start = esp_timer_get_time();                            // and makes the initial time be the current time
        }
        vTaskDelay(2); // small delay to avoid stack overflow, and give enough time for the device to read the data
    }
}
