 ...
 *
 * Adapted from Blake Felt's source code available at the link above.
 */
...
#include "global_data.h" // libraries are included in a global library

#define ADS1115_ADDR 0x48 // Sets the address of the device

/* Parameters of the analog input device (BPW34 photodiode connected to a transimpedance amplifier) */
...
#define RF 50000     // (Ω)
    ...
    typedef struct
    {
        ...
        QueueHandle_t gpio_evt_queue; // pin triggered queue
                                      /** changed queue handlers to modern pattern **/
    } ads1115_rdy_pin_t;
    ...
    void ads1115_set_max_ticks(ads1115_t *ads, TickType_t max_ticks); // maximum wait ticks for i2c bus
                                                ...
                                                /** Added freeRTOS task for initialization and data transmission **/
    void ads1115_task(void *pvParameters);      // task for sending data
