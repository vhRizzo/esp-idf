...
void app_main(void)
{
    /* Checka por problemas na memória flash do ESP */
    ...
    esp_rom_gpio_pad_select_gpio(GPIO_NUM_2);         // Sinaliza que um pino do ESP será utilizado (pino 2 e o pino padrao do LED do dev-kit ESP32S)
    ...
    ESP_ERROR_CHECK(i2c_init()); // se algum dos sensores I2C estiverem sendo usados, initializa o I2C
#endif

#ifdef ADS1115_SENSOR
    ads_queue = xQueueCreate(1, sizeof(ads_t)); // Cria uma fila para enviar itens de uma tarefa para outra
    /*
     * Cria a tarefa do conversor analógico-digital.
     * Tarefas sao funções que geralmente se mantem em um loop infinito para realizar determinadas tarefas.
     * Semelhante ao loop do Arduino, com o diferencial de que varias tarefas podem ser executadas simultaneamente.
     */
    xTaskCreatePinnedToCore(ads1115_task,             // Função da tarefa
                            "ADS1115_Task",           // Nome para controle de depuração
                            4 * 1024,                 // Tamanho da pilha de execução que será alocado para a tarefa
                            NULL,                     // Parâmetros da tarefa (geralmente não utilizado)
                            configMAX_PRIORITIES - 3, // Prioridade da tarefa (maior valor terá maior prioridade)
                            NULL,                     // Handler para esta tarefa caso seja necessário que outra tarefa ou função se comunique/controle esta
                            1);                       // Nucleo do ESP que será utilizado para executar a tarefa
#endif

    /* Daqui em diante apenas repete o mesmo processo acima para os demais sensores ou protocolos necessários */
...
#endif
    /*
     * As tarefas abaixo são de leitura e envio de dados, estas tarefas usarão as filas geradas acima para
     * receber os itens para então enviá-los via LoRaWAN, portanto é feito um atraso aqui para garantir que
     * as filas já estejam todas operantes, pois caso estas tarefas tentem ler um item de uma fila que ainda
     * não existe, um erro fatal será gerado, reiniciando o ESP.
     */
