...
Modem_SmartModular::Modem_SmartModular(uart_port_t uart_num, uint8_t pin_tx_uart, uint8_t pin_rx_uart, uint32_t baud_rate, uint8_t data_bits, uint8_t parity, uint8_t stop_bits, uint8_t flow_control, uint32_t tam_buffer){
    ...
    uart_config.data_bits = (uart_word_length_t) data_bits;
    uart_config.parity = (uart_parity_t )parity;
    uart_config.stop_bits = (uart_stop_bits_t) stop_bits;
    uart_config.flow_ctrl = (uart_hw_flowcontrol_t) flow_control;
    ...
    ESP_LOGI(__func__, "UART: %u,%u,%u,%u,%u,%u,%u,%u,%u",uart_num,pin_tx_uart,pin_rx_uart,baud_rate,data_bits,parity,stop_bits,flow_control,tam_buffer);
...
uint8_t Modem_SmartModular::ver_join(){
    ...
    esp_task_wdt_add(NULL); //WDT de 60s
    while(exito==2){
        do{
...
void Modem_SmartModular::rx_task(void *pvParameters){    
    ...
    while (1) {
        ...
        while(rxBytes!=-1){
            rxBytes=uart_read_bytes(ptr->uart_tx_rx, ( void * ) &tmp[i], 1,  100 / portTICK_RATE_MS );            
            if(rxBytes>0){
