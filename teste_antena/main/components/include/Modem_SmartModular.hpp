...
#define TX_SERIAL_AT_PIN (GPIO_NUM_14)
//TX SmartModular 13 (será RX no esp32)
#define RX_SERIAL_AT_PIN (GPIO_NUM_13)
...
class Modem_SmartModular{
    ...
    public:
        Modem_SmartModular(uart_port_t uart_num=UART_NUM_2, uint8_t tx_uart=TX_SERIAL_AT_PIN, uint8_t rx_uart=RX_SERIAL_AT_PIN, uint32_t baud_rate=9600, uint8_t data_bits=UART_DATA_8_BITS, uint8_t parity=UART_PARITY_DISABLE, uint8_t stop_bits=UART_STOP_BITS_1, uint8_t flow_control=UART_HW_FLOWCTRL_DISABLE, uint32_t tam_buffer=2048);
        ~Modem_SmartModular();
        //OTAA
